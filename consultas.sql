-- Exercicio 1
SELECT DISTINCT inscricao, nome_rua, numero_rua, bairro, cep, cidade, estado
FROM Imoveis, Enderecos
WHERE Enderecos.bairro = 'Jardim America' AND
	Imoveis.endereco = Enderecos.id
...
FROM Imoveis, Enderecos
WHERE area_total BETWEEN 100 AND 250 AND
...
FROM Contratos c, Imoveis, Enderecos
WHERE data_assinatura >= '2017-12-14' AND data_assinatura <= '2022-12-14' AND
    (c.proprietario = '11111111111' OR
    c.comprador_locatario = '11111111111'
    ) AND
...
WHERE c.imovel = '333333333333333' AND
    c.tipo = 'Aluguel' AND
    c.comprador_locatario = p.cpf

-- Exercicio 7
SELECT p.nome as nome_inquilino, c.comprador_locatario as cpf_inquilino, nome_rua, numero_rua, bairro, cep, cidade, estado, c.proprietario as cpf_proprietario
FROM Pessoas p, Imoveis, Enderecos, Contratos c
WHERE c.proprietario = '55555555555' AND
    c.tipo = 'Aluguel' AND
...
-- etc.)
SELECT inscricao, localizacao, area_total, preco_venda, preco_aluguel, Imoveis.tipo, proprietario as cpf_proprietario, Edificacoes.id as edificacao
FROM (Imoveis, Enderecos) LEFT JOIN Edificacoes ON Edificacoes.imovel = Imoveis.inscricao
WHERE nome_rua = 'Rua Jatoba' AND
    numero_rua = 13 AND
    bairro = 'Jardim America' AND
    cep = 89526778 AND
